...
### **Instalación de `tidyverse`, `readxl` y `sf`**

.pull-left[
 **Desde CRAN (recomendado para usuarios estándar)**:
   - Usar la versión estable más reciente:

```{r}
install.packages("tidyverse") # Conjunto de herramientas para ciencia de datos
install.packages("readxl")    # Para leer archivos Excel
install.packages("sf")        # Para análisis espacial
```
...
.pull-right[
 **Desde GitHub (para versiones de desarrollo)**:
   - Ideal para probar características nuevas o reportar errores.
   - Requiere el paquete `remotes`:

```{r}
install.packages("remotes")
remotes::install_github("tidyverse/tidyverse") # tidyverse en desarrollo
remotes::install_github("tidyverse/readxl")    # readxl en desarrollo
remotes::install_github("r-spatial/sf")        # sf en desarrollo
```

]

**¿Cuándo usar CRAN vs GitHub?**

- **CRAN**: Estable y ampliamente probado. Úsalo para trabajo cotidiano o producción.
- **GitHub**: Contiene características nuevas, pero podría tener más errores. Úsalo si necesitas una función específica no disponible en la versión CRAN.
...
- **`library()`**:
  - Carga el paquete y genera un error si no está instalado.
  - Recomendado para scripts y proyectos que necesitan asegurar la disponibilidad de paquetes.
  - Ejemplo:
...
- **`require()`**:
  - Carga el paquete, pero devuelve un mensaje de advertencia (en lugar de error) si no está instalado.
  - Útil en funciones condicionales o cuando el paquete es opcional.
  - Ejemplo:

```{r}
if (require(sf)) {
  print("El paquete sf está disponible.")
}
```
**Nota**: Para evitar errores, siempre asegúrate de instalar los paquetes antes de cargarlos. 😊
...
### **Exploración de una Geodatabase con `sf`**

Durante este curso vamos a utilizar la **GeoDataBase** (`.gdb`) emplada por el *Instituto Nacional de Estadísticas y Censos (INEC)*, con esta puedes realizar diversas operaciones para explorar y trabajar con sus datos. 

 **Listar las capas disponibles**:
   ```r
   library(sf)
   st_layers("ruta/carpeta_geodatabase.gdb")
   ```
   ...
   - **Salida esperada**:
     ...
     Driver: OpenFileGDB 
     Available layers:
     1: provincias           (MultiPolygon)
     2: carreteras           (LineString)
     3: puntos_interes       (Point)
     ```

 **Leer una capa específica**:
   ```r
   data <- st_read("ruta/carpeta_geodatabase.gdb", layer = "provincias")
   ```
...
```{r}
st_geometry_type(data) # Tipo de geometría (e.g., MultiPolygon)
st_crs(data)           # Sistema de referencia espacial (CRS)
```


 **Visualizar las geometrías con R-Base**:

```{r}
plot(st_geometry(data))
```

 **Guardar la capa en otro formato (e.g., GeoJSON)**:

```{r}
st_write(data, "provincias.geojson")
```

> La librería `sf` es muy útil para trabajar con datos espaciales. Puedes explorar, visualizar y exportar datos geográficos de manera sencilla. 😊 
La **geodatabase**  del INEC es una fuente de datos empleada en las distintas operaciones estaisticas del Ecuador. resulta muy útil para análisis espacial en R.
...
### Otros archivos espaciales

La función `st_read()` puede manejar múltiples formatos espaciales. Aquí tienes un resumen de los formatos más comunes, cuándo usarlos y ejemplos prácticos:

| **Formato**     | **Cuándo usarlo**                                                                                  | **Ejemplo**                                                                                       |
|------------------|--------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------|
| **Shapefile (.shp)** | Datos espaciales vectoriales más usados en SIG. Compatible con casi todas las herramientas GIS. | `st_read("ruta/archivo.shp")`                                                                   |
| **GeoJSON (.geojson)** | Intercambio de datos espaciales en la web. Ligero y basado en JSON.                           | `st_read("ruta/archivo.geojson")`                                                               |
| **KML (.kml)**        | Archivos usados en Google Earth. Para mostrar datos espaciales en aplicaciones visuales.       | `st_read("ruta/archivo.kml")`                                                                   |
...
| **Formato**     | **Cuándo usarlo**                                                                                  | **Ejemplo**                                                                                       |
|------------------|--------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------|
| **GPX (.gpx)**        | Datos de GPS. Se usa para trabajar con rutas o puntos de interés capturados por dispositivos GPS.| `st_read("ruta/archivo.gpx")`                                                                   |
| **Raster (GeoTIFF, .tif)** | Archivos raster para análisis espacial continuo (como imágenes satelitales).                 | `st_read("ruta/archivo.tif")`                                                                   |
| **CSV con coordenadas** | Datos tabulares con columnas de coordenadas (lat/lon). Convertidos a un formato espacial.      | `st_read("ruta/archivo.csv", options = c("X_POSSIBLE_NAMES=lon", "Y_POSSIBLE_NAMES=lat"))`       |
| **Well-Known Text (WKT)** | Datos espaciales definidos mediante cadenas WKT. Para compartir geometrías de manera compacta.| `st_read("ruta/archivo.wkt")`                                                                   |
...
### **Funciones más relevantes de `readxl` en combinación con el tidyverse**

El paquete **`readxl`** es ideal para leer archivos de Excel (`.xls` y `.xlsx`) directamente en R. Funciona de manera fluida con el tidyverse, permitiendo integrar datos desde Excel en flujos de análisis.

| **Función**          | **Descripción**                                                                                      | **Ejemplo de uso**                                                                                      |
|-----------------------|------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------|
| `read_excel()`       | Lee datos de un archivo Excel, soportando tanto `.xls` como `.xlsx`.                                  | `data <- read_excel("archivo.xlsx")`                                                                    |
| `excel_sheets()`     | Lista las hojas disponibles en un archivo Excel.                                                     | `hojas <- excel_sheets("archivo.xlsx")`                                                                |
| `read_xls()`         | Lee específicamente archivos en formato `.xls` (Excel 97-2003).                                       | `data <- read_xls("archivo.xls")`                                                                      |
| `read_xlsx()`        | Lee específicamente archivos en formato `.xlsx` (Excel 2007 o posterior).                             | `data <- read_xlsx("archivo.xlsx")`                                                                    |
...
### Usos de las funciones de `readxl`

Para leer un archivo Excel con varias hojas puedes listar las hojas y luego leer una específica.
...
hojas <- excel_sheets("archivo.xlsx")

# Leer una hoja específica
data <- read_excel("archivo.xlsx", sheet = "Hoja1")
```

Tambien puedes integrar datos Excel directamente en un flujo *pipes* de `dplyr`:

```{r}
library(readxl)
library(dplyr)
data <- read_excel("archivo.xlsx", sheet = 1) %>%
  filter(Categoria == "A") %>%
...
Es útil especificar tipos de datos manualmente, en caso de que `read_excel()` no detecta correctamente los tipos de datos, usa `col_types`.

```{r}
data <- read_excel("archivo.xlsx", col_types = c("text", "numeric", "date"))
```

Por ultimo, puedes combinar varias hojas de un archivo Excel en un solo data frame usando `map_df()` de `purrr` dentro del `tidyverse`.

```{r}
# Leer todas las hojas y combinarlas
all_data <- excel_sheets("archivo.xlsx") %>%
  set_names() %>%
  map_df(~ read_excel("archivo.xlsx", sheet = .x))
```

> Opciones como `range`, `skip`, `col_names` y `col_types` permiten manejar datos no limpios de manera eficiente.
...
![ESPAC](https://www.ecuadorencifras.gob.ec/wp-content/uploads/2013/10/Banner_ESPAC-01.png)
...
### Los Objetivos de la **ESPAC** 

.pull-left[

📊 **Generar información confiable y oportuna sobre:**
- 🌱 Áreas plantadas, sembradas y cosechadas.  
- 🥦 Producción y ventas de cultivos.  
- 🐄 Ganado (vacuno, porcino, ovino y otras especies).  
- 🐔 Aves de campo y de planteles avícolas.  
...
- 🌾 Cultivos permanentes y transitorios (áreas sembradas, cosechadas, producción y ventas).  
- 🐤 Existencia de aves criadas en campo y planteles avícolas.  
- 🐖 Movimientos de ganado (vacuno, porcino, ovino y otras especies).  
- 👩‍🌾 Personal agropecuario (ocasional, permanente y familiares sin remuneración).  
- 🚜 Actividades agropecuarias realizadas en los terrenos.  
- ☠️ Uso de plaguicidas, grado de exposición y afectaciones.  
- 🌱 Tipos de fertilizantes utilizados en cultivos.
...
📋 **Tipo de operación**: Encuesta.  
🌍 **Cobertura geográfica**: Nacional y provincial (excepto Galápagos).  

]

.pull-right[
📈 **Desagregación**: Nacional, regional, provincial, tipo de suelo, cultivos, ganado, prácticas agrícolas.  
🔄 **Periodicidad**: Anual, continua desde 2002.  

]

![](https://scontent.fuio1-2.fna.fbcdn.net/v/t39.30808-6/242691649_4845849612105995_8730662397570944506_n.jpg?_nc_cat=101&ccb=1-7&_nc_sid=127cfc&_nc_ohc=KZm2RoD_tbMQ7kNvgE2b431&_nc_oc=Adh_K_75K_7T4ZqSGvr32SkRVHEyY0GqhyGryE3-h1ox7eNooD0d4NGyXMTDQPjqU3GObg0ALQYDjFPk-17bV2kd&_nc_zt=23&_nc_ht=scontent.fuio1-2.fna&_nc_gid=AWKxL8vxfEDbHOQUEizASzX&oh=00_AYB4Ld0rOos9C7tkIrHKLp6fHBrHWS1a-lb9s4rmtBpkVg&oe=67835FAD)
